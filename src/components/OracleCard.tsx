...
export function OracleCardView({ card, onDrawAgain, onDrawNew, readOnly, showCanDraw = true }: Props) {
  ...
  const ActionButton = ({
    ...
    <button
      onClick={onClick}
      className={
        "text-[12px] uppercase tracking-[0.18em] font-medium px-5 py-2.5 rounded-full transition-colors " +
        (active
  ...
  return (
    <article className="relative group animate-card-rise">
      ...
      <div className="relative rounded-2xl p-7 sm:p-8 border border-dawn-haze/15 bg-dawn-surface/80 backdrop-blur-xl shadow-[0_40px_120px_-30px_rgba(245,180,120,0.35),inset_0_1px_0_rgba(255,220,180,0.08)]">
        ...
        <p className="text-base italic font-serif opacity-75 leading-relaxed text-pretty">{card.opener}</p>
        ...
        <div className="animate-message-unfold">
          <div className="mt-4 space-y-3 max-w-[46ch]">
            {card.message.split(/\n{2,}/).map((para, i) => (
              <p key={i} className="text-dawn-ink/75 leading-relaxed text-base text-pretty">{para}</p>
            ))}
          </div>

          {card.reflection && (
            <div className="mt-6 pl-4 border-l-2 border-dawn-rose/40">
              <p className="text-[12px] uppercase tracking-[0.2em] font-medium text-dawn-rose/70 mb-1.5">Reflection</p>
              <p className="text-base font-serif italic text-dawn-ink/85 leading-relaxed text-pretty max-w-[44ch]">{card.reflection}</p>
            </div>
          )}
        </div>
        ...
        {sparkOpen && (
          <div className="mt-5 p-5 bg-dawn-sky/60 border border-dawn-haze/15 rounded-2xl space-y-4">
            <label className="block text-[12px] uppercase tracking-[0.18em] font-medium opacity-75">Add a note (optional)</label>
            <textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={140} rows={2}
              ...
              className="w-full bg-dawn-night/60 text-dawn-ink placeholder:text-dawn-ink/45 border border-dawn-haze/15 rounded-xl p-4 text-base focus:outline-none focus:ring-1 ring-dawn-rose/30 resize-none" />
            <div className="flex items-center gap-4">
              <button onClick={doShare}
                className="text-[12px] uppercase tracking-[0.18em] font-bold px-6 py-2.5 bg-dawn-rose text-dawn-sky rounded-full hover:bg-dawn-haze transition-colors">
                {copied ? "Link copied" : "Share"}
              </button>
              <button onClick={() => setSparkOpen(false)} className="text-[12px] uppercase tracking-[0.18em] opacity-70 hover:opacity-80 transition-opacity">Cancel</button>
            </div>
          </div>
        )}
      </div>
      ...
      {!readOnly && !followUpUsed && busy && (
        <div className="mt-6 flex flex-col items-center text-center py-8">
          ...
          <p className="font-serif italic text-dawn-haze/80 text-base animate-card-rise">Reading your card…</p>
        </div>
      )}

      {!readOnly && !followUpUsed && !busy && (
        <form onSubmit={submitFollowUp} className="mt-6">
          <label className="block text-[12px] uppercase tracking-[0.18em] font-medium opacity-70 mb-3 ml-1">What would you like to know more about?</label>
          ...
          <div className="relative">
            <input id={`fu-${card.id}`} value={followUp} onChange={(e) => setFollowUp(e.target.value)}
              ...
              className="w-full bg-dawn-surface/70 text-dawn-ink placeholder:text-dawn-ink/45 border border-dawn-haze/15 rounded-xl px-5 py-4 pr-24 text-base focus:outline-none focus:ring-1 ring-dawn-rose/30" />
            <button type="submit"
              className="absolute right-2 top-1/2 -translate-y-1/2 text-[12px] uppercase tracking-[0.18em] font-bold px-4 py-2 bg-dawn-rose text-dawn-sky rounded-full hover:bg-dawn-haze transition-colors">
              Ask
            </button>
          </div>
        </form>
      ...
      {followCard && (
        <div className="mt-8">
          <p className="text-[12px] uppercase tracking-[0.18em] font-medium opacity-70 mb-3 ml-1">The card answered</p>
