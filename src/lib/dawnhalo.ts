...
const dailyCards: Omit<OracleCard, "id" | "createdAt">[] = [
  { art: "dawn",  title: "The Gentle Return",  opener: openers[0], message: "You don't need to rush into the noise just yet. Today asks only for your presence, not your productivity." },
  { art: "silk",  title: "The Still Point",    opener: openers[1], message: "There is a deep, quiet strength in moving at the pace of your own breath. Let the day come to meet you." },
  { art: "flame", title: "Small Steady Light", opener: openers[2], message: "You do not need to burn brighter. You only need to keep burning. That is more than enough today." },
  { art: "bloom", title: "What Wants to Open", opener: openers[3], message: "Something tender is reaching toward the light in you. Notice it without rushing to name it." },
  { art: "moon",  title: "The Soft Knowing",   opener: openers[4], message: "You already know. Trust the version of you who's been quietly paying attention all along." },
  { art: "hand",  title: "The Held Hour",      opener: openers[5], message: "You are not carrying it alone, even when it feels that way. Set something down for an hour and see." },
];
...
const responses: Record<Exclude<Intent, "crisis">, Omit<OracleCard, "id" | "createdAt" | "prompt">[]> = {
  question: [
    { art: "moon", opener: "Let me lean in — your question has weight to it…", title: "The Patient Choice",
    ...
    { art: "dawn", opener: "Mm. This card stepped forward the moment you asked…", title: "Two Doors, One You",
      message: "Either path holds something for you. The real question is which version of yourself you want to practice being." },
  ],
  feeling: [
    { art: "hand", opener: "Let me sit with what you just said for a moment…", title: "It Makes Sense",
    ...
    { art: "silk", opener: "I'm catching something tender underneath this…", title: "Soften, Don't Solve",
  ...
  loneliness: [
    { art: "bloom", opener: "I want to read this one slowly with you…", title: "You Are Seen Here",
  ...
  appearance: [
    { art: "flame", opener: "Let me move past the surface of what you said…", title: "Tend the Light, Not the Lamp",
  ...
  general: [
    { art: "dawn", opener: "Here — this is the card that wanted to be seen today…", title: "A Little Light",
      message: "You're doing better than you think. Take this with you into the next small thing." },
    { art: "moon", opener: "Mm. The deck settled on this one quickly…", title: "Trust the Quiet Lead",
...
export function encodeShare(card: OracleCard, note?: string): string {
  const payload = { t: card.title, o: card.opener, m: card.message, a: card.art, n: note || "" };
  return btoa(unescape(encodeURIComponent(JSON.stringify(payload))));
}
export function decodeShare(token: string): { title: string; opener: string; message: string; art: CardArt; note: string } | null {
  try {
    const o = JSON.parse(decodeURIComponent(escape(atob(token))));
    return { title: o.t, opener: o.o, message: o.m, art: o.a, note: o.n || "" };
  } catch { return null; }
}
