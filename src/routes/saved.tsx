    ...
    <div className="min-h-screen bg-dawn-sky">
      <main className="max-w-md mx-auto px-6 pt-12 pb-32">
        <header className="mb-10">
          <p className="text-[10px] uppercase tracking-[0.2em] font-medium opacity-50 mb-1">Your collection</p>
          <h1 className="text-3xl font-serif font-light tracking-tight italic">Saved cards.</h1>
        </header>

        {open ? (
          <div>
            <button onClick={() => setOpen(null)} className="mb-6 text-[10px] uppercase tracking-[0.18em] opacity-60">← Back to collection</button>
            <OracleCardView card={open} readOnly />
          </div>
        ) : cards.length === 0 ? (
          <div className="p-8 border border-dashed border-dawn-ink/15 rounded-2xl text-center">
            <p className="font-serif italic text-lg opacity-70">Your collection is quiet for now.</p>
            <p className="mt-2 text-sm opacity-50">Tap <span className="font-medium">Collect</span> on a card to keep it here.</p>
          </div>
        ) : (
